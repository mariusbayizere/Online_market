...
//         if (orderOpt.isEmpty()) {
//             return ResponseEntity.status(404).body(ApiResponses.error("Order not found", 404));
//         }

//         return ResponseEntity.ok(ApiResponses.success("Order retrieved successfully", 200, orderOpt.get()));
//     }
...
//         if (userOpt.isEmpty()) {
//             return ResponseEntity.status(404).body(new ApiResponses("User not found", 404));
//         }
//         order.setUser(userOpt.get());

//         Optional<Products> productOpt = productsRepository.findById(order.getProduct().getProduct_ID());
//         if (productOpt.isEmpty()) {
//             return ResponseEntity.status(404).body(new ApiResponses("Product not found", 404));
//         }
//         Products product = productOpt.get();
//         order.setProduct(product);

//         if (product.getProduct_Quantity() == 0) {
//             return ResponseEntity.status(400).body(new ApiResponses(
...
//         return ResponseEntity.status(201).body(new ApiResponses(
...
//     public ResponseEntity<?> updateOrder(@PathVariable int id, @RequestBody Orders updatedOrder) {
...
//         return ResponseEntity.ok(ApiResponses.success("Order updated successfully", 200, savedOrder));
//     }
...
//         return ResponseEntity.ok(ApiResponses.success("Order deleted successfully", 200, null));
//     }
...
//      */
//     @GetMapping("/track/{id}")
//     public ResponseEntity<?> trackOrderStatus(@PathVariable int id) {
//         Optional<Orders> orderOpt = ordersRepository.findById(id);
//         if (orderOpt.isEmpty()) {
//             return ResponseEntity.status(404).body(ApiResponses.error("Order not found", 404));
//         }
...
//         return ResponseEntity.ok(ApiResponses.success("Order status retrieved successfully", 200, order.getOrder_Status()));
//     }
...
//         return ResponseEntity.ok(ApiResponses.success("Orders retrieved successfully", 200, orders));
//     }
...
public class OrdersController {
     ...
     */
    // @GetMapping("/track/{id}")
    // @Operation(summary = "Track order status", description = "Retrieves the current status of an order")
    // @io.swagger.v3.oas.annotations.responses.ApiResponses(value = {
    //     @ApiResponse(responseCode = "200", description = "Order status retrieved successfully"),
    //     @ApiResponse(responseCode = "404", description = "Order not found")
    // })
    // public ResponseEntity<?> trackOrderStatus(
    //         @Parameter(description = "ID of the order to track") @PathVariable int id) {
    //     Optional<Orders> orderOpt = ordersRepository.findById(id);
    //     if (orderOpt.isEmpty()) {
    //         return ResponseEntity.status(404).body(com.example.Project_Online_market.response.ApiResponses.error("Order not found", 404));
    //     }
    ...
    //     Orders order = orderOpt.get();
    //     return ResponseEntity.ok(com.example.Project_Online_market.response.ApiResponses.success("Order status retrieved successfully", 200, order.getOrder_Status()));
    // }
